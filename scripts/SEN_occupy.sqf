        ...
        waitUntil {
            ...
            if (_pos distance SEN_centerPos <= SEN_range && {!(surfaceIsWater _pos)} && {!([_pos, "SEN_safezone_mrk"] call SEN_fnc_checkInMarker)}) then {
                if (random 1 > ((paramsArray select 6)*.01)) then {
                    ...
                    [_grp,_pos,(SEN_range*0.08 max 1000), "MOVE", "AWARE", "YELLOW", "NORMAL", (["COLUMN", "STAG COLUMN"] call BIS_fnc_selectRandom), "this spawn CBA_fnc_searchNearby", [2,6,12]] call CBA_fnc_taskPatrol;
                } else {
