...
#define DEFAULT_FIELDREPAIR_EACH_PART_TIME 50
#define DEFAULT_FIELDREPAIR_EACH_HARDPART_TIME 55
#define DEFAULT_FULLREPAIR_LENGTH 65
...
#define STR_NEED_TOOLKIT "Free field repair used. Need toolkit."
...
zlt_prc_repairvehicle = {
    ...
    if ( _hastk isEqualTo 0 ) exitWith {STR_NEED_TOOLKIT call zlt_fnc_notify;};
    ...
    if (_repairFinished) then {
        _hastk = [] call zlt_fnc_hastk;
        if (_hastk isEqualTo 0) exitWith {STR_NEED_TOOLKIT call zlt_fnc_notify; sleep 1.;};
        STR_REPAIR_FINISHED call zlt_fnc_notify;
        [_veh,"zlt_fnc_partrepair", _veh] call bis_fnc_MP;
        //if (_hastk isEqualTo 1) then {player removeItem "ToolKit";};
        //if (_hastk isEqualTo 2) then { ["ToolKit",_veh] call zlt_fnc_removeitemfromcargo;};
...
zlt_fnc_heavyRepair = {
    ...
    if (not alive player or vehicle player isEqualTo player or speed _veh > 3 or _veh distance _truck > 15 ) exitWith {STR_REPAIR_CONDITIONS call zlt_fnc_notify;};
...
zlt_fnc_heavyRepairCOnd = {
    ...
    if (_isRepair isEqualTo -1) exitWith {false};

    _res = _veh distance _truck <= 15 and {player isEqualTo (driver _truck)} and {(_veh isKindOf "LandVehicle" or _veh isKindOf "Ship" or _veh isKindOf "Air")} and {alive _truck} and {alive player} and {alive _veh} and {not zlt_mutexAction} and {speed _veh <= 3} and {(damage _veh != 0)};
    _res;
...
if (isNil "zlt_cancelActionId") then {
    ...
    player addAction["<t color='#FF9900'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,true,"","canMove (vehicle player) and ((vehicle player) isKindOf 'Wheeled_APC_F') and player isEqualTo driver (vehicle player) and surfaceIsWater getpos (vehicle player)  and abs(speed (vehicle player) ) < 3 and not zlt_mutexAction"];
