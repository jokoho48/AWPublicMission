...
_stance = call {
    if (SEN_approvalCiv <= -250) exitWith {
        "The local population does not support you!"
    };
    if (SEN_approvalCiv > -250 && {SEN_approvalCiv <= -100}) exitWith {
        "The local population is displeased with you."
    };
    if (SEN_approvalCiv > -100 && {SEN_approvalCiv <= 100}) exitWith {
        "The local population is indifferent to you."
    };
    if (SEN_approvalCiv > 100 && {SEN_approvalCiv <= 250}) exitWith {
        "The local population is pleased with you."
    };
    if (SEN_approvalCiv > 250) exitWith {
        "The local population supports you!"
    };
    "The local population is indifferent to you."
};
