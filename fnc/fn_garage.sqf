        ...
        switch _idc do {
            ...
            case IDC_RSCDISPLAYGARAGE_TAB_STATIC: {
                ...
                if (getnumber (_cfg >> "isUAV") < 1) then {
                    ...
                    _cargoIndexes = [];
                    _cargoProxyIndexes = getarray (_cfg >> "cargoProxyIndexes");
                    _transportSoldier = getnumber (_cfg >> "transportSoldier");
                    {
                        if (_x in _cargoProxyIndexes && count _cargoIndexes < _transportSoldier) then {_cargoIndexes pushback (_foreachindex + 1)};
                    } foreach getarray (_cfg >> "getInProxyOrder");
                    if (count _cargoIndexes == 0) then {
                        for "_i" from 1 to getnumber (_cfg >> "transportsoldier") do {
                            if !(_i in _proxyIndexes) then {_cargoIndexes pushback _i;};
                        };
                    };
                    {
                    ...
                    } foreach _cargoIndexes;
                    _ctrlButtonTry ctrlenable (true && _fullVersion);
                } else {
            ...
            case IDC_RSCDISPLAYGARAGE_TAB_SUBCREW: {
                ...
                _roleIndex = _roleIndex - 1; //--- Value was increased by 1 to allow negatvie values, revert back
...
*/
    ...
    case "SetCrewStatus": {
        ...
        for "_i" from 0 to (lbsize _ctrlListCrew - 1) do {
            ...
            if (_isPlayer) then {
                ...
                _ctrlListCrew lbsetpicture [_i,_checkboxTextures select (!isnull _unit && !_locked)];
            };
        };
        {
    ...
    case "showTemplates": {
        ...
        for "_i" from 0 to (count _savedData - 1) step 2 do {
            ...
            _categoryIndex = -1;
            {if (_vehModel in _x) exitwith {_categoryIndex = _foreachindex;}} foreach _data;
