...
if (_type in _uav) then {
    ...
    [[player,"AinvPercMstpSrasWrflDnon_Putdown_AmovPercMstpSrasWrflDnon"],"QS_fnc_switchMoveMP",nil,false] call BIS_fnc_MP;
    [_t] spawn {
        private "_t";
        _t = _this select 0;
        sleep 2;
        createVehicleCrew _t;
        broadcastCrew = crew _t;
        publicVariableServer "broadcastCrew";
        crewServer = crew _t;
        publicVariableServer "crewServer";
        sleep 1;
