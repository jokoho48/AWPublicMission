[] spawn {
    ...
    if (!isNil "CHVD_scriptRunning" && {CHVD_scriptRunning}) exitWith {systemChat "CHVD script is running. Addon disabled"};

    //Define variables, load from profileNamespace
    CHVD_allowNoGrass = missionNameSpace getVariable ["CHVD_allowNoGrass", true];
    CHVD_maxView = missionNamespace getVariable ["CHVD_maxView", 12000];
    CHVD_maxObj = missionNamespace getVariable ["CHVD_maxObj", 12000];
