...
#define OBJVEH_TYPES "rhs_tigr_msv"
#define ALTVEH_TYPES "rhs_tigr_msv"
#define OBJUNIT_TYPES "rhs_msv_officer","rhs_msv_officer_armored","rhs_msv_rifleman","rhs_msv_rifleman","rhs_msv_marksman"
...
//-------------------------------------------------------------------------- CREATE GROUP, VEHICLE AND UNIT
	...
	"rhs_msv_engineer" createUnit [_flatPos1, _aGroup];
...
//--------------------------------------------------------------------------- BRIEFING
	...
	sideMarkerText = "Intel sichern"; publicVariable "sideMarkerText";
	"sideMarker" setMarkerText "Nebenmission: Intel sichern"; publicVariable "sideMarker"; 	
	_briefing = "<t align='center'><t size='2.2'>Neue Nebenmission</t><br/><t size='1.5' color='#00B2EE'>Intel sichernl</t><br/>____________________<br/>Wir haben Informationen von der Zivilbevölkerungen erhalten, dass bei einem Treffen hoher Funktionäre der Feindkräfte, geheime, strategisch wichtige Dokumente den Besitzer wechseln sollen. Dies ist unsere Chance!<br/><br/>Wir haben die Position auf der Karte markiert; rückt auf die Position vor und sichert die Daten. Die Daten sollten sich in einem der Fahrzeuge befinden. Möglicherweise werden die Daten jedoch auch von einem Funktionär am Mann getragen.</t>";
	GlobalHint = _briefing; hint parseText GlobalHint; publicVariable "GlobalHint";
	showNotification = ["NewSideMission", "Intel sichern"]; publicVariable "showNotification";
	sideMarkerText = "Intel sichern"; publicVariable "sideMarkerText";
...
while { sideMissionUp } do {
	...
	if (!alive _intelObj) exitWith {
		...
		hqSideChat = "Daten zerstört! Mission FEHLGESCHLAGEN!"; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
	...
	if (NOT_ESCAPING) then {
		...
		if (_intelObj call BIS_fnc_enemyDetected) then {
			...
			hqSideChat = "Feindkräfte haben Sie aufgeklärt und versuchen nun mit den Daten zu fliehen!"; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
	...
	if (HE_ESCAPED) exitWith {
			...
			hqSideChat = "Feind geflohen! Mission FEHLGESCHLAGEN!"; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
			sideMissionUp = false; publicVariable "sideMissionUp";
			[] spawn QS_fnc_SMhintFAIL;
			{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["sideMarker", "sideCircle"]; publicVariable "sideMarker";
	...
	if (SM_SUCCESS) exitWith {
		...
		hqSideChat = "Daten gesichert. Mission erfolgreich abgeschlossen! Wir senden die Daten zur Analyse ins HQ."; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
		sideMissionUp = false; publicVariable "sideMissionUp";
