...
DISARM_ADD_ACTION = {
    ...
    _itemRequirement = "(_target distance _this < 3) && (vehicle player == player)";
    {
        _itemRequirement = format["%1 && ((items player) find ""%2"" > -1)", _itemRequirement, _x];
        nil
    } count itemsRequiredToDisarm;
    _itemRequirement = compile _itemRequirement;
...
DISARM_ACTION = {
    ...
    if (((random 100) < _chance)) then {
        ...
        _sectionName call INCREMENT_DISARM_COUNTER;
        _args call PREPARE_IED_FOR_CLEANUP;
        ...
        sleep 4.545;
        disableUserInput false;
        _sectionNamecall INCREMENT_EXPLOSION_COUNTER;
        _args call EXPLOSIVESEQUENCE_DISARM;
